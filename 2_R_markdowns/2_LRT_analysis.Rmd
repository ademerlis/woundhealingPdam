...
This Rmarkdown contains the downstream analyses of the DDS object using the Likelihood Ratio Test (LRT). This analysis involves comparing the variance between the full DDS model (~condition + hour + condition:hour) and the reduced model (~condition + hour) to see if there are gene expression patterns in the wounded corals over time (while controlling for any initial differences observed at hour 0).

Analyses include graphs of normalized counts of significant genes and a PCA. All other downstream analyses are not an option because the total number of DGEs is too low (N=3).
...
```{r}
dds_LRT <- readRDS(file = "rds_files/ddsLRT.rds")
...
mutate(hour = as.numeric(as.character(hour))) %>% 
  ...
  labs(x="Hour", y="Normalized counts", title ="Galectin-3-binding protein B (pdam_00001007)") #looked at list of annotated genes for this info
```
...
mutate(hour = as.numeric(as.character(hour))) %>% 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="aceA: Isocitrate lyase (pdam_00008890)") #aceA: Isocitrate lyase (from Pdam_Gene_Names_Info.csv)

plotCounts(dds_LRT, gene = "pdam_00004156", 
                   intgroup = c("hour","condition"), returnData = TRUE) %>% 
mutate(hour = as.numeric(as.character(hour))) %>% 
  ggplot(.,
  aes(x = hour, y = count, color = condition, group = condition)) + 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="Gene pdam_00004156 (unknown function)") #unknown function of gene
```

# look at Peroxidasin homolog specifically to see its expression pattern
```{r}
plotCounts(dds_LRT, gene = "pdam_00000345", 
                   intgroup = c("hour","condition"), returnData = TRUE) %>% 
mutate(hour = as.numeric(as.character(hour))) %>% 
  ggplot(.,
  aes(x = hour, y = count, color = condition, group = condition)) + 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="PXDN: Peroxidasin homolog (pdam_00000345)")

plotCounts(dds_LRT, gene = "pdam_00000346", 
                   intgroup = c("hour","condition"), returnData = TRUE) %>% 
mutate(hour = as.numeric(as.character(hour))) %>% 
  ggplot(.,
  aes(x = hour, y = count, color = condition, group = condition)) + 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="PXDN: Peroxidasin homolog (pdam_00000346)")

plotCounts(dds_LRT, gene = "pdam_00000349", 
                   intgroup = c("hour","condition"), returnData = TRUE) %>% 
mutate(hour = as.numeric(as.character(hour))) %>% 
  ggplot(.,
  aes(x = hour, y = count, color = condition, group = condition)) + 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="PXDN: Peroxidasin homolog (pdam_00000349)")
```
# look at matrix metalloproteinase
```{r}
plotCounts(dds_LRT, gene = "pdam_00003559", 
                   intgroup = c("hour","condition"), returnData = TRUE) %>% 
mutate(hour = as.numeric(as.character(hour))) %>% 
  ggplot(.,
  aes(x = hour, y = count, color = condition, group = condition)) + 
  ...
  theme_classic() +
  scale_color_manual(values = c("lightblue", "red")) +
  labs(x="Hour", y="Normalized counts", title ="MMP16: Matrix metalloproteinase-16 (pdam_00003559)")
```
