...
res_GO_h1_up <- as.data.frame(res_h1) %>%
  drop_na(padj) %>% 
  mutate(Group=case_when(padj<0.05 & log2FoldChange > 0 ~"1", TRUE ~ "0")) #going to try doing log2FC of 2 instead of 0 to see if we get more specific GO terms
...
#label differentially expressed genes as '1' and non-significant genes as '0'
res_GO_h2_up <- as.data.frame(res_h2) %>%
  drop_na(padj) %>% 
  mutate(Group=case_when(padj<0.05 & log2FoldChange > 0 ~"1", TRUE ~ "0")) #selecting log2fold change level of 0

res_GO_h2_up %>% filter(Group == "1") #6 genes

#create the gene universe
gene_universe_h2_up <-as.numeric(res_GO_h2_up$Group)
length(gene_universe_h2_up) #13,254 genes
gene_universe_h2_up <-factor(gene_universe_h2_up)
names(gene_universe_h2_up) <- rownames(res_GO_h2_up) 

#label differentially expressed genes as '1' and non-significant genes as '0'
res_GO_h2_down <- as.data.frame(res_h2) %>%
  drop_na(padj) %>% 
  mutate(Group=case_when(padj<0.05 & log2FoldChange < 0 ~"1", TRUE ~ "0")) #selecting log2fold change level of 0

res_GO_h2_down %>% filter(Group == "1") #10 genes

#create the gene universe
gene_universe_h2_down <-as.numeric(res_GO_h2_down$Group)
length(gene_universe_h2_down) #13,254 genes
gene_universe_h2_down <-factor(gene_universe_h2_down)
names(gene_universe_h2_down) <- rownames(res_GO_h2_down) 
```
...
#label differentially expressed genes as '1' and non-significant genes as '0'
res_GO_h4_up <- as.data.frame(res_h4) %>%
  drop_na(padj) %>% 
  mutate(Group=case_when(padj<0.05 & log2FoldChange > 0 ~"1", TRUE ~ "0")) #selecting log2fold change level of 0

res_GO_h4_up %>% filter(Group == "1") #19 genes

#create the gene universe
gene_universe_h4_up <-as.numeric(res_GO_h4_up$Group)
length(gene_universe_h4_up) #18591 genes
gene_universe_h4_up <-factor(gene_universe_h4_up)
names(gene_universe_h4_up) <- rownames(res_GO_h4_up) 

#label differentially expressed genes as '1' and non-significant genes as '0'
res_GO_h4_down <- as.data.frame(res_h4) %>%
  drop_na(padj) %>% 
  mutate(Group=case_when(padj<0.05 & log2FoldChange < 0 ~"1", TRUE ~ "0")) #selecting log2fold change level of 0

res_GO_h4_down %>% filter(Group == "1") #27 genes

#create the gene universe
gene_universe_h4_down <-as.numeric(res_GO_h4_down$Group)
length(gene_universe_h4_down) #18591 genes
gene_universe_h4_down <-factor(gene_universe_h4_down)
names(gene_universe_h4_down) <- rownames(res_GO_h4_down) 
```
...
length(sigGenes((GO_MF_h1))) #34 genes
...
showSigOfNodes(GO_MF_h2,score(GO_MF_h2_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')
...
splitting up- and down-regulated genes based on log2fold change for hour 2

### Up-regulated genes in hour 2
```{r}
#create topGO data object
GO_MF_h2_up <-new("topGOdata",
                ontology="MF",
                allGenes=gene_universe_h2_up,
                nodeSize=10,
                annotationFun=annFUN.gene2GO,
                gene2GO = geneID_GO)

# tests to identify enriched gene ontology terms
GO_MF_h2_up_resultFisherclassic<-runTest(GO_MF_h2_up,algorithm = "classic", statistic = "fisher")

#generate results table
GO_MF_h2_up_resultstable<-GenTable(GO_MF_h2_up,
                                Fisherclassic = GO_MF_h2_up_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_MF_h2_up,score(GO_MF_h2_up_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_MF_h2_up_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=2) %>% 
  mutate(regulation_direction = "Up") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_MF_h2_up #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```

### Down-regulated genes in hour 2
```{r}
#create topGO data object
GO_MF_h2_down <-new("topGOdata",
                ontology="MF",
                allGenes=gene_universe_h2_down,
                nodeSize=10,
                annotationFun=annFUN.gene2GO,
                gene2GO = geneID_GO)

# tests to identify enriched gene ontology terms
GO_MF_h2_down_resultFisherclassic<-runTest(GO_MF_h2_down,algorithm = "classic", statistic = "fisher")

#generate results table
GO_MF_h2_down_resultstable<-GenTable(GO_MF_h2_down,
                                Fisherclassic = GO_MF_h2_down_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_MF_h2_down,score(GO_MF_h2_down_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_MF_h2_down_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=2) %>% 
  mutate(regulation_direction = "down") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_MF_h2_down #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```
...
showSigOfNodes(GO_MF_h4,score(GO_MF_h4_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')
...
splitting up- and down-regulated genes based on log2fold change for hour 4

### Up-regulated genes in hour 4
```{r}
#create topGO data object
GO_MF_h4_up <-new("topGOdata",
                ontology="MF",
                allGenes=gene_universe_h4_up,
                nodeSize=10,
                annotationFun=annFUN.gene2GO,
                gene2GO = geneID_GO)

# tests to identify enriched gene ontology terms
GO_MF_h4_up_resultFisherclassic<-runTest(GO_MF_h4_up,algorithm = "classic", statistic = "fisher")

#generate results table
GO_MF_h4_up_resultstable<-GenTable(GO_MF_h4_up,
                                Fisherclassic = GO_MF_h4_up_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_MF_h4_up,score(GO_MF_h4_up_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_MF_h4_up_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=4) %>% 
  mutate(regulation_direction = "Up") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_MF_h4_up #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```

### Down-regulated genes in hour 4
```{r}
#create topGO data object
GO_MF_h4_down <-new("topGOdata",
                ontology="MF",
                allGenes=gene_universe_h4_down,
                nodeSize=10,
                annotationFun=annFUN.gene2GO,
                gene2GO = geneID_GO)

# tests to identify enriched gene ontology terms
GO_MF_h4_down_resultFisherclassic<-runTest(GO_MF_h4_down,algorithm = "classic", statistic = "fisher")

#generate results table
GO_MF_h4_down_resultstable<-GenTable(GO_MF_h4_down,
                                Fisherclassic = GO_MF_h4_down_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_MF_h4_down,score(GO_MF_h4_down_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_MF_h4_down_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=4) %>% 
  mutate(regulation_direction = "down") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_MF_h4_down #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```



## dot plot figure of all GO terms each hour up and down
```{r}
...
listOfDataFrames <- list(sigGO_MF_h1_up, sigGO_MF_h1_down, sigGO_MF_h2_up, sigGO_MF_h2_down, sigGO_MF_h4_up, sigGO_MF_h4_down)

GO_MF_allhours_UpvsDown <- do.call("rbind",listOfDataFrames) 
GO_MF_allhours_UpvsDown$Fisherclassic <- as.numeric(GO_MF_allhours_UpvsDown$Fisherclassic)
GO_MF_allhours_UpvsDown$hour <- as.factor(GO_MF_allhours_UpvsDown$hour)

#write_csv(GO_MF_allhours_UpvsDown, "../results/GO_MF_allhours_UpvsDown.csv")
...
#now split it by Up and down regulated

GO_MF_allhours_UpvsDown %>% 
  mutate(GO_term=str_c(GO.ID,Term, sep=" ")) %>% 
  ggplot(data=., aes(x=-log(Fisherclassic), y=reorder(GO_term, -Fisherclassic))) +
    geom_point(aes(size=Significant, color=-log(Fisherclassic))) +
    scale_color_viridis(option="plasma") +
  labs(x="-log P-Value", y="Gene Ontology Term Description", title=paste("Significant Molecular Functions"), color = "-log(P-value)") +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 50)) +
  theme_bw() +
  facet_grid(vars(hour,regulation_direction), scales = "free") + 
...
showSigOfNodes(GO_BP_h1,score(GO_BP_h1_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')
...
# tests to identify enriched gene ontology terms
GO_BP_h1_up_resultFisherclassic<-runTest(GO_BP_h1_up,algorithm = "classic", statistic = "fisher")

#generate results table
GO_BP_h1_up_resultstable<-GenTable(GO_BP_h1_up,
                                Fisherclassic = GO_BP_h1_up_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_BP_h1_up,score(GO_BP_h1_up_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_BP_h1_up_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=1) %>% 
  mutate(regulation_direction = "Up") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_BP_h1_up #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```
...
# tests to identify enriched gene ontology terms
GO_BP_h1_down_resultFisherclassic<-runTest(GO_BP_h1_down,algorithm = "classic", statistic = "fisher")

#generate results table
GO_BP_h1_down_resultstable<-GenTable(GO_BP_h1_down,
                                Fisherclassic = GO_BP_h1_down_resultFisherclassic,
                                topNodes = 50)

showSigOfNodes(GO_BP_h1_down,score(GO_BP_h1_down_resultFisherclassic),firstSigNodes = 5,useInfo = 'all')

GO_BP_h1_down_resultstable %>% 
  filter(Significant >= 1) %>% 
  mutate(hour=1) %>% 
  mutate(regulation_direction = "down") %>% 
  dplyr::select(GO.ID:Fisherclassic, hour, regulation_direction) %>% 
  filter(Fisherclassic < 0.05) -> sigGO_BP_h1_down #filter for genes from this time point that are significantly associated with the GO terms (which are also significantly enriched)
```
